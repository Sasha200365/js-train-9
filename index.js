...
function findElementGreaterThan(arr, num) {
  ...
  let foundElement = arr.find((number) => number > num);
  ...
  //Якщо так повертаємо елемент
    ...
    return "рядок Числа яке задовільняе умову не знайдено";
  }
...
function findIndexByProperty(arr, name) {
  ...
  let foundIndex = arr.findIndex(({ name }) => name === "Petro");
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArrayString = arrSlice.join(" ");
  // Повертаємо рядок.
...
function getArrayKeys(arr) {
  ...
  let arrKeys = Array.from(keyIter);
  {
...
function getPositiveArrayValues(arr) {
  ...
  for (const elem of iter) {
    ...
    function positiveArrayValues(elem) {
      return elem > 0;
    }
    let arrOk = arr.filter(positiveArrayValues);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrSplice = arr.splice(0, startIdx);
  // За допомогою методу map() подвоюємо кожен елемент масиву.
  const doubledArray = arrSplice.map((el) => el * 2);
...
function wordLengths(arr) {
  ...
  const arrMap = arr.map((word) => word.length);
...
function insertElementAt(arr, index, element) {
  ...
  const arrSplice1 = arr.splice(index, 0, element);
...
function expandArrayByN(arr, n) {
  ...
  //   const arrFlatMap = arr.flatMap((el) => {
  //     return el.map((it) => {
  //       return repeat(it[0], n);
  //     });
  //   });
...
function calculateTotalPrice(arr) {
  ...
  const totalPriceZero = arr.reduce(
...
function findLastNegativeNumberIndex(arr) {
  ...
  const arrLastIndex = arr.findLastIndex((number) => number);
